var COUNTRY = ["Albania", "Algeria", "Angola", "Argentina", "Armenia", "Australia", "Austria", "Azerbaijan", "Bahamas", "Bangladesh", "Barbados", "Belarus", "Belgium", "Belize", "Benin", "Bosnia and Herzegovina", "Brazil", "Bulgaria", "Burkina Faso", "Cambodia", "Cameroon", "Canada", "Central African Republic", "Chile", "China", "Colombia", "Costa Rica", "Croatia", "Cyprus", "Czech Republic", "Denmark", "Dominican Republic", "Ecuador", "Egypt", "El Salvador", "Estonia", "Ethiopia", "Finland", "France", "Gabon", "Georgia", "Germany", "Ghana", "Greece", "Guatemala", "Guinea", "Guyana", "Haiti", "Honduras", "Hungary", "Iceland", "India", "Indonesia", "Iran", "Ireland", "Italy", "Jamaica", "Jordan", "Kenya", "Kiribati", "Latvia", "Lithuania", "Luxembourg", "Madagascar", "Malawi", "Malaysia", "Maldives", "Mali", "Mauritania", "Mauritius", "Mexico", "Mongolia", "Montenegro", "Morocco", "Mozambique", "Namibia", "Nepal", "Netherlands", "Nicaragua", "Niger", "Norway", "Pakistan", "Paraguay", "Peru", "Philippines", "Poland", "Portugal", "Romania", "Rwanda", "Senegal", "Serbia", "Singapore", "Slovenia", "South Africa", "Spain", "Sri Lanka", "Sudan", "Suriname", "Sweden", "Switzerland", "Tajikistan", "Togo", "Tunisia", "Turkey", "Uganda", "Ukraine", "United Arab Emirates", "United Kingdom", "United States", "Uruguay", "Uzbekistan", "Zambia", "Zimbabwe"]

var MCC_MNC =
{
    "Afghanistan": [
        "Afghan Wireless Communication Company",
        "Telecom Development Company Afghanistan Ltd",
        "MTN Group Afghanistan",
        "WASEL Afghanistan",
        "Afghan Telecom"
    ],
    "Albania": [
        "Telekom Albania",
        "Vodafone Albania",
        "Albtelecom",
        "Plus Communication"
    ],
    "Andorra": [
        "Servei De Tele DAndorra"
    ],
    "Angola": [
        "UNITEL Sarl",
        "MOVICEL Telecommunications SA"
    ],
    "Anguilla": [
        "Digicel",
        "Cable & Wireless"
    ],
    "Antigua and Barbuda": [
        "Antigua Public Utilities Authority",
        "Cable & Wireless Caribbean Cellular (Antigua) Limited",
        "AT&T Wireless"
    ],
    "Argentina": [
        "NII Holdings",
        "AMX Argentina SA",
        "Telecom Personal SA"
    ],
    "Aruba": [
        "Servicio di Telecomunicacion di Aruba",
        "Digicel Aruba"
    ],
    "Australia": [
        "Telstra Corporation Limited",
        "Singtel Optus Proprietary Limited",
        "Vodafone Hutchison Australia Proprietary Limited",
        "Department of Defence",
        "Vodafone Network Pty Ltd",
        "OneTel Limited",
        "Norfolk Telecom",
        "Telstra Corporation Ltd",
        "Railcorp, Transport for New South Wales",
        "TPG Telecom",
        "Victorian Rail Track",
        "Optus",
        "Pactel International Pty Ltd",
        "Lycamobile Pty Ltd",
        "Ausgrid Corporation",
        "Queensland Rail Limited",
        "iiNet Ltd",
        "Challenge Networks Pty Ltd",
        "Advanced Communications Technologies Pty Ltd",
        "Pilbara Iron Company Services Pty Ltd",
        "Dialogue Communications Pty Ltd",
        "Nexium Telecommunications",
        "RCOM International Pty Ltd",
        "Compatel Limited",
        "BHP Billiton",
        "Thales Australia",
        "CLX Networks Pty Ltd",
        "Santos Limited",
        "MessageBird Pty Ltd",
        "Optus Mobile Pty Ltd",
        "Yancoal Australia Ltd",
        "Truphone Pty Ltd",
        "CITIC Pacific Mining",
        "Aqura Technologies Pty",
        "Groote Eylandt Mining Company Pty Ltd",
        "Arrow Energy Pty Ltd",
        "Roy Hill Iron Ore Pty Ltd",
        "Clermont Coal Operations Pty Ltd",
        "Pivotel Group Pty Limited",
        "Commtel Network Solutions Pty Ltd",
        "National Broadband Network Co",
        "OneTel"
    ],
    "Austria": [
        "A1 Telekom Austria",
        "T-Mobile Austria GmbH",
        "Hutchison Drei Austria",
        "T-Mobile Austria",
        "Lycamobile Austria",
        "Mundio Mobile Austria",
        "MASS Response Service GmbH",
        "smartspace GmbH",
        "MTEL Austrija GmbH"
    ],
    "Azerbaijan": [
        "CATEL",
        "Azerfon",
        "Special State Protection Service of the Republic of Azerbaijan"
    ],
    "Bahamas": [
        "The Bahamas Telecommunications Company Ltd (BaTelCo)",
        "Cable Bahamas Ltd"
    ],
    "Bahrain": [
        "Bahrain Telecommunications Company",
        "Zain Bahrain",
        "Civil Aviation Authority",
        "STC Bahrain"
    ],
    "Bangladesh": [
        "Grameenphone Ltd",
        "Axiata Bangladesh Ltd",
        "Banglalink Digital Communications Ltd"
    ],
    "Barbados": [
        "LIME (formerly known as Cable & Wireless)",
        "Sunbeach Communications"
    ],
    "Belarus": [
        "A1 Belarus",
        "Mobile TeleSystems",
        "BelCel",
        "Belarusian Telecommunications Network",
        "Beltelecom",
        "Belorussian Cloud Technologies"
    ],
    "Belgium": [
        "Belgacom Mobile",
        "Infrabel",
        "Telenet",
        "Lycamobile sprl",
        "Mundio Mobile Belgium nv",
        "Nethys",
        "Orange SA",
        "Dense Air Belgium SPRL",
        "BICS",
        "Unleashed NV",
        "Ericsson NV",
        "JOIN Experience (Belgium)",
        "IP Nexia"
    ],
    "Benin": [
        "Benin Telecoms Mobile",
        "Telecel Benin",
        "Spacetel Benin"
    ],
    "Bhutan": [
        "B-Mobile / Bhutan Telecom Ltd",
        "Tashi InfoComm Limited"
    ],
    "Bolivia": [
        "Nuevatel PCS De Bolivia SA",
        "Entel SA"
    ],
    "Bosnia and Herzegovina": [
        "Public Enterprise Croatian Telecom Ltd",
        "RS Telecommunications JSC Banja Luka",
        "BH Telecom"
    ],
    "Botswana": [
        "Mascom Wireless (Pty) Limited",
        "Orange (Botswana) Pty Limited",
        "Botswana Telecommunications Corporation"
    ],
    "Brazil": [
        "NII Holdings, Inc",
        "Telecom Italia Mobile",
        "Claro",
        "Sercomtel Celular",
        "Brasil Telecom GSM",
        "Datora (Vodafone)",
        "Amazonia Celular",
        "TNL PCS Oi",
        "Algar Telecom SA",
        "Unicel"
    ],
    "British Virgin Islands": [
        "Cable & Wireless",
        "BVI Cable TV Ltd",
        "Caribbean Cellular Telephone",
        "Digicel (BVI) Limited"
    ],
    "Brunei Darussalam": [
        "Data Stream Technology Sdn Bhd"
    ],
    "Bulgaria": [
        "A1 Bulgaria",
        "Telenor (Bulgaria)"
    ],
    "Burkina Faso": [
        "Orange Burkina Faso",
        "Telecel Faso SA"
    ],
    "Burundi": [
        "VTEL MEA",
        "Onatel",
        "LACELL SU"
    ],
    "Cambodia": [
        "Smart Axiata Co Ltd",
        "Cambodia Advance Communications Co Ltd",
        "Viettel",
        "The Royal Group"
    ],
    "Cameroon": [
        "Mobile Telephone Network Cameroon Ltd",
        "Orange Cameroun SA"
    ],
    "Canada": [
        "Xplornet Communications",
        "Telus Mobility",
        "ALO Mobile Inc",
        "Bragg Communications",
        "Airtel Wireless",
        "Ambra Solutions",
        "Rogers Communications",
        "Execulink",
        "Keewaytinook Okimakanak Mobile",
        "Dryden Mobility",
        "ABC Allen Business Communications Ltd",
        "Shaw Communications",
        "Videotron",
        "Lynx Mobility",
        "LightSquared",
        "Quadro Communications Co-op",
        "Iristel",
        "Bell Mobility",
        "ICE Wireless",
        "Bell Aliant",
        "Thunder Bay Telephone",
        "Bell MTS",
        "CityWest",
        "SaskTel Mobility",
        "TerreStar Networks",
        "TNW Wireless Inc",
        "Shared Telus, Bell, and SaskTel",
        "Wightman Telecom",
        "Ericsson Canada"
    ],
    "Cayman Islands": [
        "Cable & Wireless (Cayman Islands) Limited"
    ],
    "Central African Rep": [
        "Atlantique Telecom Centrafrique SA",
        "Telecel Centrafrique",
        "Orange RCA"
    ],
    "Chad": [
        "Bharti Airtel SA",
        "SotelTchad"
    ],
    "Chile": [
        "Claro Chile SA",
        "Novator Partners",
        "Multikom SA",
        "Blue Two Chile SA",
        "VTR SA",
        "Celupago SA",
        "Tribe Mobile Chile SPA",
        "Cibeles Telecom SA",
        "Nomade Telecomunicaciones SA",
        "COMPATEL Chile Limitada",
        "Empresas Bunker SA",
        "Inversiones Santa Fe Limitada",
        "Cellplus SpA",
        "Claro Servicios Empresariales S A"
    ],
    "China": [
        "China Mobile",
        "China Unicom"
    ],
    "Colombia": [
        "Colombia Telecomunicaciones SA ESP",
        "Edatel SA ESP",
        "Une EPM Telecomunicaciones SA ESP",
        "Empresas Municipales de Cali",
        "COMCEL SA",
        "Bellsouth Colombia",
        "Avantel SAS"
    ],
    "Comoros": [
        "Comores Telecom",
        "Telecom Malagasy (Telma)"
    ],
    "Congo": [
        "Celtel Congo",
        "MTN CONGO SA"
    ],
    "Costa Rica": [
        "Instituto Costarricense de Electricidad",
        "Virtualis SA"
    ],
    "Croatia": [
        "T-Hrvatski Telekom",
        "Tele2",
        "A1 Hrvatska"
    ],
    "Cuba": [
        "Empresa de Telecomunicaciones de Cuba, SA"
    ],
    "Cyprus": [
        "Cyprus Telecommunications Authority",
        "Monaco Telecom",
        "PrimeTel PLC",
        "Lemontel Ltd",
        "Mundio Mobile Cyprus Ltd"
    ],
    "Czech Rep": [
        "T-Mobile Czech Republic",
        "O2 Czech Republic",
        "Vodafone Czech Republic",
        "Nordic Telecom sro",
        "PODA as",
        "Nordic Telecom 5G as",
        "Compatel sro"
    ],
    "Dem Rep of the Congo": [
        "Vodacom Congo RDC sprl",
        "Airtel sprl",
        "Supercell SPRL",
        "Orange RDC sarl",
        "Yozma Timeturns sprl",
        "Africell RDC sprl"
    ],
    "Denmark": [
        "TDC A/S",
        "Telenor Denmark",
        "Syniverse Technologies",
        "Dansk Beredskabskommunikation A/S",
        "Hi3G Denmark ApS",
        "Voxbone mobile",
        "Lycamobile Denmark Ltd",
        "Compatel Limited",
        "Monty UK Global Limited",
        "Ice Danmark ApS",
        "Tismi BV",
        "Cubic Telecom",
        "Telia",
        "Banedanmark",
        "SMS Provider Corp",
        "LINK Mobile A/S",
        "Interactive digital media GmbH",
        "Greenwave Mobile IoT ApS",
        "Onomondo ApS"
    ],
    "Dominica": [
        "Cable & Wireless"
    ],
    "Dominican Rep": [
        "Altice Group",
        "Trilogy Dominicana, SA"
    ],
    "Ecuador": [
        "Otecel SA",
        "CONECEL SA"
    ],
    "Egypt": [
        "Orange Egypt",
        "Vodafone Egypt",
        "Etisalat Egypt"
    ],
    "El Salvador": [
        "CTE Telecom Personal, SA de CV",
        "Digicel, SA de CV",
        "Telemovil El Salvado"
    ],
    "Equatorial Guinea": [
        "GETESA"
    ],
    "Estonia": [
        "Telia Eesti",
        "Elisa Eesti",
        "Tele2 Eesti",
        "OY Top Connect",
        "Telia Eesti AS",
        "Premium Net International SRL Eesti filiaal",
        "Siseministeerium (Ministry of Interior)"
    ],
    "Ethiopia": [
        "Ethio Telecom"
    ],
    "Faroe Islands": [
        "Faroese Telecom",
        "Vodafone Faroe Islands",
        "Tosa Sp/F"
    ],
    "Fiji": [
        "Vodafone Fiji",
        "Digicel Fiji",
        "Telecom Fiji Ltd"
    ],
    "Finland": [
        "DNA Oy",
        "Elisa Oyj",
        "Nokia Solutions and Networks Oy",
        "Traficom",
        "Satakunnan ammattikorkeakoulu Oy",
        "Liikennevirasto",
        "Compatel Ltd",
        "Teknologian tutkimuskeskus VTT Oy",
        "Ukko Mobile Oy",
        "Voxbone SA",
        "Suomen Virveverkko Oy",
        "Bittium Wireless Oy",
        "Ukkoverkot Oy",
        "Telia Finland Oyj / Suomen Yhteisverkko Oy",
        "Tismi BV",
        "Telia Finland Oyj",
        "TeliaSonera Finland Oyj"
    ],
    "France": [
        "Orange SA",
        "MobiquiThings",
        "Societe d'ingenierie systeme telecom et reseaux",
        "Altice",
        "Iliad",
        "Local Exchange Global Operation Services",
        "Bouygues Telecom",
        "Transatel",
        "LycaMobile",
        "Coriolis Telecom",
        "Syma Mobile",
        "Mundio Mobile",
        "EDF",
        "Weaccess group",
        "SEM@FOR77",
        "Fondation b-com",
        "Halys"
    ],
    "French Departments and Territories in the Indian Ocean": [
        "BJT Partners",
        "Telco OI",
        "Zeop mobile"
    ],
    "French Polynesia": [
        "VITI",
        "Mara Telecom",
        "Pacific Mobile Telecom",
        "Onati SAS"
    ],
    "Gabon": [
        "Airtel Gabon SA",
        "USAN Gabon SA"
    ],
    "Gambia": [
        "Gamcel",
        "Africell",
        "Comium",
        "QCell Gambia",
        "Gamtel-Ecowan"
    ],
    "Georgia": [
        "Silknet",
        "MagtiCom",
        "Mobitel",
        "JSC Compatel",
        "GlobalCell",
        "Gmobile",
        "Premium Net International SRL",
        "Mobilive"
    ],
    "Germany": [
        "Vodafone D2 GmbH",
        "Telekom Deutschland GmbH",
        "Telogic Germany GmbH",
        "NetCologne",
        "Inquam Deutschland",
        "Voiceworks GmbH",
        "Multiconnect GmbH",
        "sipgate Wireless GmbH",
        "Chaos Computer Club",
        "Lycamobile",
        "Ericsson GmbH",
        "Nokia",
        "Qualcomm CDMA Technologies GmbH",
        "Core Network Dynamics GmbH"
    ],
    "Ghana": [
        "MTN Group",
        "Kasapa / Hutchison Telecom",
        "Netafrique Dot Com Ltd"
    ],
    "Gibraltar": [
        "Gibtelecom",
        "Eazitelecom"
    ],
    "Greece": [
        "COSMOTE - Mobile Telecommunications SA",
        "OTE",
        "OSE",
        "Vodafone Greece",
        "AMD Telecom",
        "Wind Hellas Telecommunications SA",
        "interConnect",
        "Yuboto",
        "Compatel Limited",
        "CYTA",
        "BWS",
        "Inter Telecom"
    ],
    "Greenland": [
        "TELE Greenland A/S",
        "inu:it a/s"
    ],
    "Grenada": [
        "Cable & Wireless Grenada Ltd"
    ],
    "Guadeloupe": [
        "Outremer Telecom",
        "Dauphin Telecom",
        "Free Mobile"
    ],
    "Guatemala": [
        "Telecomunicaciones de Guatemala, SA"
    ],
    "Guinea": [
        "Orange SA",
        "Sotelgui Lagui",
        "Cellcom"
    ],
    "Guyana": [
        "Guyana Telephone & Telegraph Co",
        "Quark Communications Inc",
        "U-Mobile (Cellular) Inc",
        "eGovernment Unit, Ministry of the Presidency"
    ],
    "Haiti": [
        "Communication Cellulaire d'Haiti SA",
        "Unigestion Holding SA",
        "NATCOM SA"
    ],
    "Honduras": [
        "Servicios de Comunicaciones de Honduras SA de CV",
        "Celtel",
        "Digicel de Honduras"
    ],
    "Hong Kong, China": [
        "CSL Limited",
        "CITIC Telecom 1616",
        "Hutchison Telecom",
        "SmarTone Mobile Communications Limited",
        "China Unicom (Hong Kong) Limited",
        "Truphone Limited",
        "China Motion Telecom",
        "China-Hong Kong Telecom",
        "China Mobile Hong Kong Company Limited",
        "PCCW",
        "PCCW-HKT",
        "21Vianet Mobile Ltd",
        "263 Mobile Communications (HongKong) Limited",
        "Multibyte Info Technology Ltd",
        "Hong Kong Government",
        "China Data Enterprises Ltd",
        "China Telecom Global Limited",
        "Hong Kong Broadband Network Ltd"
    ],
    "Hungary": [
        "MVM Net Ltd",
        "DIGI Telecommunication Ltd",
        "Invitech ICT Services Ltd",
        "Magyar Telekom Plc",
        "UPC Hungary Ltd"
    ],
    "Iceland": [
        "Iceland Telecom",
        "Og fjarskipti hf",
        "IMC Island ehf",
        "IceCell ehf"
    ],
    "India": [
        "Haryana",
        "Punjab",
        "Himachal Pradesh",
        "Delhi & NCR",
        "Gujarat",
        "Andhra Pradesh and Telangana",
        "Assam",
        "Uttar Pradesh (East)",
        "North East",
        "West Bengal",
        "Kerala",
        "Mumbai",
        "Maharashtra & Goa",
        "Bihar",
        "Orissa",
        "Kolkata",
        "Bihar & Jharkhand",
        "Jammu & Kashmir",
        "Chennai",
        "Tamil Nadu",
        "Karnataka",
        "Unknown",
        "Uttar Pradesh (West)",
        "Madhya Pradesh & Chhattisgarh",
        "Rajasthan",
        "Karnataka (Bangalore)",
        "Madhya Pradesh & Chattishgarh",
        "Andaman Nicobar",
        "Vodafone Punjab",
        "Maharashtra",
        "Madhya Pradesh",
        "Bihar/Jharkhand",
        "Delhi"
    ],
    "Indonesia": [
        "PT Pasifik Satelit Nusantara (ACeS)",
        "PT Indonesian Satellite Corporation Tbk (INDOSAT)",
        "PT Natrindo Telepon Seluler",
        "PT Telekomunikasi Selular",
        "PT XL Axiata Tbk",
        "PT Mobile-8 Telecom"
    ],
    "Iraq": [
        "Asia Cell Telecommunications Company",
        "Zain Iraq",
        "Telecom Ltd",
        "Itisaluna Wireless CO",
        "Omnnea Wireless"
    ],
    "Ireland": [
        "Vodafone Ireland",
        "Eir Group plc",
        "Liffey Telecom",
        "Lycamobile",
        "UPC",
        "Carphone Warehouse",
        "Cubic Telecom Limited"
    ],
    "Israel": [
        "Partner Communications Company Ltd",
        "Cellcom Israel Ltd",
        "Pelephone Communications Ltd",
        "Globalsim Ltd",
        "Wataniya Palestine Ltd (Ooredoo)",
        "Hot Mobile Ltd",
        "Golan Telecom Ltd",
        "Marathon 018 Xphone Ltd",
        "365 Telecom",
        "Free Telecom",
        "Ituran Cellular Communications",
        "Alon Cellular Ltd",
        "Home Cellular Ltd",
        "Gale Phone",
        "Cellact Communications Ltd",
        "019 Communication Services Ltd / TELZAR",
        "Bezeq The Israeli Telecommunication Corp Ltd",
        "BIP Communications Ltd",
        "Beezz Communication Solutions Ltd",
        "Israel Ministry of Defense",
        "LB Annatel Ltd",
        "PHI Networks",
        "CG Networks"
    ],
    "Italy": [
        "Telecom Italia SpA",
        "Vodafone Italia SpA",
        "Wind Tre",
        "BLU SpA"
    ],
    "Jamaica": [
        "Symbiote Investment Limited",
        "Digicel (Jamaica) Limited",
        "Cable & Wireless Communications"
    ],
    "Japan": [
        "UQ Communications Inc",
        "Hanshin Cable Engineering Co, Ltd",
        "Japan Radio Company, Ltd",
        "Wireless City Planning Inc",
        "SAKURA Internet Inc",
        "LTE-X, Inc",
        "NTT DoCoMo, Inc",
        "Okinawa Cellular Telephone"
    ],
    "Jordan": [
        "Jordan Mobile Telephone Services",
        "XPress Telecom",
        "Umniah Mobile Company",
        "Petra Jordanian Mobile Telecommunications Company (MobileCom)"
    ],
    "Kazakhstan": [
        "KaR-Tel LLP",
        "Kcell JSC"
    ],
    "Kenya": [
        "Safaricom Limited",
        "Bharti Airtel",
        "Mobile Pay Kenya Limited",
        "Essar Telecom Kenya",
        "Finserve Africa Limited",
        "Telkom Kenya",
        "Sema Mobile Services Limited",
        "Homeland Media Group Limited",
        "Jamii Telecommunications Limited",
        "WiAfrica Kenya Limited"
    ],
    "Kiribati": [
        "Telecom Services Kiribati Ltd",
        "OceanLink"
    ],
    "Kosovo*": [
        "Telecom of Kosovo JSC",
        "IPKO",
        "DardaphoneNet LLC"
    ],
    "Kuwait": [
        "Zain Kuwait",
        "National Mobile Telecommunications",
        "Saudi Telecom Company"
    ],
    "Kyrgyzstan": [
        "Sky Mobile LLC",
        "7 Mobile",
        "Alfa Telecom CJSC",
        "NurTelecom LLC",
        "Saima Telecom",
        "iTel"
    ],
    "Lao PDR": [
        "Lao Telecom",
        "Enterprise of Telecommunications Lao",
        "VimpelCom Lao Ltd"
    ],
    "Latvia": [
        "Latvian Mobile Telephone",
        "Tele2",
        "Telekom Baltija",
        "Beta Telecom",
        "Bite Latvija",
        "Rigatta",
        "SIA \"MEGATEL\"",
        "IZZI",
        "Camel Mobile"
    ],
    "Lebanon": [
        "Ogero Telecom"
    ],
    "Lesotho": [
        "Vodacom Lesotho (Pty) Ltd",
        "Econet Ezi-cel"
    ],
    "Liberia": [
        "Novafone Inc"
    ],
    "Liechtenstein": [
        "Swisscom AG",
        "Swisscom Schweiz AG",
        "Salt Liechtenstein AG",
        "Telecom Liechtenstein AG",
        "Cubic Telecom AG",
        "First Mobile AG",
        "EMnify GmbH",
        "Soracom LI Ltd"
    ],
    "Lithuania": [
        "Telia Lietuva",
        "UAB Tele2 (Tele2 AB, Sweden)"
    ],
    "Luxembourg": [
        "POST Luxembourg",
        "MTX Connect Sarl",
        "Blue Communications",
        "Tango SA",
        "Interactive digital media GmbH",
        "Mitto AG",
        "E-Lux Mobile Telecommunication SA",
        "Orange SA"
    ],
    "Macao, China": [
        "China Telecom (Macau) Company Limited",
        "Hutchison Telephone (Macau), Limitada",
        "China Telecom (Macau) Limitada"
    ],
    "Madagascar": [
        "Bharti Airtel",
        "Orange Madagascar SA",
        "Telma Mobile SA"
    ],
    "Malawi": [
        "Telecom Network Malawi",
        "Airtel Malawi Limited"
    ],
    "Malaysia": [
        "Maxis, DiGi, Celcom, XOX",
        "Celcom Axiata Berhad",
        "DiGi Telecommunications",
        "Maxis Communications Berhad",
        "U Mobile Sdn Bhd",
        "Electcoms Berhad"
    ],
    "Maldives": [
        "Dhivehi Raajjeyge Gulhun"
    ],
    "Mali": [
        "Malitel SA",
        "Orange Mali SA",
        "Alpha Telecommunication Mali SA"
    ],
    "Malta": [
        "Vodafone Malta",
        "YOM Ltd",
        "Mobile Communications Limited",
        "Melita"
    ],
    "Mauritania": [
        "Mattel",
        "Chinguitel",
        "Mauritel Mobiles"
    ],
    "Mauritius": [
        "Cellplus Mobile Communications Ltd",
        "Emtel Ltd"
    ],
    "Mexico": [
        "Comunicaciones Digitales Del Norte, SA de CV",
        "AT&T Mexico",
        "Servicios de Acceso Inalambrico, SA de CV"
    ],
    "Monaco": [
        "Monaco Telecom"
    ],
    "Mongolia": [
        "Mobicom Corporation"
    ],
    "Montenegro": [
        "Telenor Montenegro",
        "T-Mobile Montenegro LLC",
        "MTEL CG"
    ],
    "Montserrat": [
        "Cable & Wireless"
    ],
    "Morocco": [
        "Ittissalat Al-Maghrib (Maroc Telecom)",
        "Wana Corporate",
        "Al Houria Telecom"
    ],
    "Mozambique": [
        "Mocambique Celular SA",
        "Movitel, SA"
    ],
    "Myanmar": [
        "Myanmar Posts and Telecommunications",
        "Myanmar Economic Corporation",
        "Ooredoo Myanmar",
        "Telenor Myanmar",
        "Myanmar National Tele & Communication Co, Ltd",
        "Amara Communication Co, Ltd",
        "Fortune Telecom Co, Ltd",
        "Global Technology Co, Ltd"
    ],
    "Namibia": [
        "MTC Namibia",
        "Telecom Namibia",
        "Paratus Telecommunications (Pty)",
        "Demshi Investments CC",
        "MTN Namibia"
    ],
    "Nauru": [
        "Digicel (Nauru) Corporation"
    ],
    "Netherlands": [
        "RadioAccess Network Services",
        "Tele2 Nederland BV",
        "Voiceworks BV",
        "Vodafone Libertel BV",
        "Elephant Talk Communications Premium Rate Services",
        "Mundio Mobile (Netherlands) Ltd",
        "Teleena (MVNE)",
        "KPN Mobile The Netherlands BV",
        "Lycamobile Netherlands Limited",
        "KPN BV",
        "Unica Installatietechniek BV",
        "Ziggo BV",
        "T-Mobile Netherlands BV",
        "Intercity Mobile Communications BV",
        "Mixe Communication Solutions BV",
        "Ministerie van Defensie",
        "Wyless Nederland BV",
        "Private Mobility Nederland BV",
        "CapX BV",
        "SpeakUp BV",
        "Lancelot BV",
        "Private Mobile Ltd",
        "Nextgen Mobile Ltd",
        "BodyTrace Netherlands BV",
        "Zetacom BV",
        "AGMS Netherlands BV",
        "Utility Connect BV",
        "Koning en Hartman BV",
        "Roamware (Netherlands) BV"
    ],
    "New Caledonia": [
        "OPT New Caledonia"
    ],
    "New Zealand": [
        "Telecom New Zealand",
        "Vodafone New Zealand",
        "Woosh Wireless",
        "Spark New Zealand",
        "Bluereach Limited",
        "2degrees"
    ],
    "Nicaragua": [
        "Servicios de Comunicaciones SA"
    ],
    "Niger": [
        "Bharti Airtel Limited",
        "Atlantique Telecom (subsidiary of Etisalat)"
    ],
    "Nigeria": [
        "Bharti Airtel Limited",
        "MTN Nigeria Communications Limited",
        "Nigerian Mobile Telecommunications Limited",
        "Globacom Ltd",
        "Emerging Markets Telecommunication Services Ltd"
    ],
    "Niue": [
        "Telecom Niue"
    ],
    "Norway": [
        "Telenor Norge AS",
        "Televerket AS",
        "ICE Norge AS",
        "Phonero AS",
        "Com4 AS",
        "Norwegian Communications Authority",
        "SystemNet AS",
        "ICE Communication Norge AS",
        "Jernbaneverket AS",
        "Lyca Mobile Ltd",
        "TampNet AS"
    ],
    "Oman": [
        "Oman Telecommunications Company",
        "Omani Qatari Telecommunications Company SAOC"
    ],
    "Pakistan": [
        "Mobilink-PMCL",
        "Pakistan Telecommunication Mobile Ltd",
        "China Mobile",
        "Telenor Pakistan",
        "WaridTel"
    ],
    "Palau": [
        "Palau National Communications Corp",
        "Palau Equipment Company Inc"
    ],
    "Panama": [
        "Cable & Wireless Panama SA",
        "Digicel Group"
    ],
    "Papua New Guinea": [
        "Bemobile Limited",
        "Telikom PNG Ltd",
        "Digicel PNG"
    ],
    "Paraguay": [
        "Hola Paraguay SA",
        "AMX Paraguay SA"
    ],
    "Philippines": [
        "Globe Telecom",
        "Digital Telecommunications Philippines"
    ],
    "Poland": [
        "Polkomtel Sp z oo",
        "T-Mobile Polska SA",
        "Aero 2 Sp z oo",
        "P4 Sp z oo",
        "Netia SA",
        "E-Telko Sp z oo",
        "Lycamobile Sp z oo",
        "Nordisk Polska Sp z oo",
        "Cyfrowy Polsat SA",
        "Move Telecom SA",
        "Telco Leaders Ltd",
        "AMD Telecom SA",
        "SIA NetBalt",
        "Exteri Sp z oo",
        "Arcomm Sp z oo",
        "PGE Systemy SA",
        "IT Partners Telco Sp z oo",
        "Polskie Sieci Radiowe Sp z oo Sp ka",
        "Advanced Technology & Experience Sp z oo",
        "SIA Ntel Solutions",
        "PhoneNet Sp z oo",
        "Interfonica Sp z oo",
        "GrandTel Sp z oo",
        "Phone IT Sp z oo",
        "Compatel Limited",
        "Truphone Poland Sp z oo"
    ],
    "Portugal": [
        "Vodafone Portugal",
        "LycaMobile",
        "Compatel, Limited",
        "Infraestruturas de Portugal, SA",
        "G9Telecom, SA"
    ],
    "Qatar": [
        "ooredoo",
        "Ministry of Interior"
    ],
    "Romania": [
        "Telekom Romania",
        "RCS&RDS",
        "Enigma-System"
    ],
    "Russian Federation": [
        "Mobile TeleSystems",
        "MegaFon PJSC",
        "Nizhegorodskaya Cellular Communications",
        "Sibchallenge",
        "Yeniseytelecom",
        "Zao SMARTS",
        "Dontelekom",
        "Scartel",
        "Baykal Westcom / New Telephone Company / Far Eastern Cellular",
        "Kuban GSM",
        "Miatel",
        "INDIGO",
        "Tele2",
        "Beeline",
        "ROSTELECOM",
        "Stavtelesot / North Caucasian GSM",
        "Primtelefon",
        "Telecom XXI",
        "OJSC Vimpel-Communications"
    ],
    "Rwanda": [
        "MTN Rwandacell SARL",
        "Airtel RWANDA",
        "Olleh Rwanda Networks"
    ],
    "Saint Helena, Ascension and Tristan da Cunha": [
        "Sure South Atlantic Ltd"
    ],
    "Saint Kitts and Nevis": [
        "Cable & Wireless St Kitts & Nevis Ltd"
    ],
    "Saint Lucia": [
        "Cable & Wireless"
    ],
    "Saint Pierre and Miquelon": [
        "GLOBALTEL"
    ],
    "Saint Vincent and the Grenadines": [
        "Cable & Wireless (St Vincent & the Grenadines) Ltd"
    ],
    "Samoa": [
        "Digicel Pacific Ltd",
        "Bluesky Samoa Ltd"
    ],
    "San Marino": [
        "San Marino Telecom"
    ],
    "Saudi Arabia": [
        "Saudi Telecom Company",
        "Etihad Etisalat Company"
    ],
    "Senegal": [
        "Sonatel",
        "Sudatel",
        "CSU-SA"
    ],
    "Serbia": [
        "Telenor Serbia",
        "Telekom Srbija",
        "VIP Mobile",
        "Orion Telekom",
        "MUNDIO MOBILE doo",
        "GLOBALTEL doo"
    ],
    "Seychelles": [
        "Cable & Wireless Seychelles",
        "Telecom Seychelles Ltd"
    ],
    "Sierra Leone": [
        "Orange SL Limited",
        "Lintel Sierra Leone Limited",
        "Comium (Sierra Leone) Ltd",
        "Qcell Sierra Leone",
        "InterGroup Telecom SL",
        "Mobitel",
        "Datatel (SL) Ltd"
    ],
    "Singapore": [
        "Singapore Telecom",
        "M1 Limited",
        "StarHub Mobile",
        "Liberty Wireless Pte Ltd",
        "TPG Telecom Pte Ltd",
        "GRID Communications Pte Ltd"
    ],
    "Slovakia": [
        "Orange Slovensko",
        "Slovak Telekom"
    ],
    "Slovenia": [
        "COMPATEL Ltd",
        "A1 Slovenija",
        "Telekom Slovenije",
        "T-2 doo"
    ],
    "Solomon Islands": [
        "BMobile (SI) Ltd"
    ],
    "South Africa": [
        "Vodacom",
        "Telkom SA SOC Ltd",
        "Sasol (Pty) Ltd",
        "Sentech (Pty) Ltd",
        "Cell C (Pty) Ltd",
        "MTN Group",
        "South African Police ServiceGauteng",
        "Neotel Pty Ltd",
        "Phoenix System Integration (Pty) Ltd",
        "Sishen Iron Ore Company (Ltd) Pty",
        "Wireless Business Solutions (Pty) Ltd",
        "Cape Town Metropolitan Council",
        "SMSPortal (Pty) Ltd",
        "Wirels Connect",
        "A to Z Vaal Industrial Supplies Pty Ltd",
        "Hymax Talking Solutions (Pty) Ltd",
        "Bokamoso Consortium",
        "Karabo Telecoms (Pty) Ltd",
        "Ilizwi Telecommunications",
        "Bokone Telecoms Pty Ltd",
        "Kingdom Communications Pty Ltd",
        "Amatole Telecommunications Pty Ltd",
        "South African Police Service",
        "SMS Cellular Services (Pty) Ltd",
        "Ericsson South Africa (Pty) Ltd",
        "Integrat (Pty) Ltd",
        "Lycamobile (Pty) Ltd",
        "Airports Company South Africa"
    ],
    "Spain": [
        "Vodafone Spain",
        "Alta Tecnologia en Comunicacions SL",
        "Orange Espagne SAU",
        "Xfera Moviles SA",
        "ZINNIA TELECOMUNICACIONES, SLU",
        "TELECOM CASTILLA-LA MANCHA, SA",
        "SAC CONVERGENT AGGREGATION SERVICES, SLU",
        "R Cable y Telecomunicaciones Galicia SA",
        "Best Spain Telecom",
        "LycaMobile SL",
        "SCN Truphone, SL",
        "Consorcio de Telecomunicaciones Avanzadas, SA",
        "Xfera Moviles SAU",
        "Compatel Limited",
        "Red Digital De Telecomunicaciones de las Islas Baleares, SL",
        "INGENIUM OUTSOURCING SERVICES, SL",
        "ALAI OPERADOR DE TELECOMUNICACIONES, SL",
        "Administrador de Infraestructuras Ferroviarias"
    ],
    "Sri Lanka": [
        "Dialog Axiata PLC",
        "Etisalat Lanka (Pvt) Ltd"
    ],
    "Sudan": [
        "Zain Group - Sudan",
        "MTN Sudan",
        "Canar Telecom"
    ],
    "Suriname": [
        "Telecommunications Company Suriname (Telesur)",
        "Digicel Group Limited"
    ],
    "Sweden": [
        "Telia Sverige AB",
        "HI3G Access AB",
        "Netett Sverige AB",
        "3G Infrastructure Services AB",
        "Telenor Sverige AB",
        "Tele2 Sverige AB",
        "Communication for Devices in Sweden AB",
        "ComHem AB",
        "Lycamobile Sweden Limited",
        "Tele2 Business AB",
        "42 Telecom AB",
        "Generic Mobile Systems Sweden AB",
        "Mundio Mobile (Sweden) Limited",
        "Wireless Maingate Messaging Services AB",
        "EuTel AB",
        "Infobip Limited (UK)",
        "Monty UK Global Ltd",
        "Twilio Sweden AB",
        "GlobeTouch AB",
        "LINK Mobile A/S",
        "Mercury International Carrier Services",
        "NextGen Mobile Ltd",
        "RebTel Network AB",
        "Compatel Limited",
        "Mobile Arts AB",
        "42 Telecom LTD",
        "interactive digital media GmbH",
        "CLX Networks AB",
        "Voxbone mobile",
        "Borderlight AB",
        "Netmore Group AB",
        "Shyam Telecom UK Ltd",
        "Telenor Connexion AB",
        "MobiWeb Ltd",
        "Telenabler AB",
        "Spirius AB",
        "SMS Provider Corp",
        "Tismi BV",
        "MessageBird BV"
    ],
    "Switzerland": [
        "Swisscom AG",
        "Sunrise Communications AG",
        "Salt Mobile SA",
        "Comfone AG",
        "SBB AG",
        "TelCommunication Services AG",
        "Swisscom Broadcast AG",
        "relario AG",
        "UPC Schweiz GmbH",
        "Lycamobile AG",
        "WeMobile SA",
        "Mitto AG",
        "Beeone Communications SA",
        "Compatel Ltd"
    ],
    "Syrian Arab Republic": [
        "Syriatel Mobile Telecom",
        "MTN Syria",
        "Syrian Telecom"
    ],
    "Tajikistan": [
        "JV Somoncom",
        "Indigo Tajikistan",
        "TT Mobile",
        "Babilon-Mobile",
        "Tacom"
    ],
    "Tanzania": [
        "MIC Tanzania Limited",
        "Zanzibar Telecom Ltd",
        "Bharti Airtel",
        "Tanzania Telecommunication Company LTD (TTCL)",
        "Benson Informatics Limited",
        "Smile Telecoms Holdings Ltd",
        "Wiafrica Tanzania Limited"
    ],
    "Thailand": [
        "CAT Telecom",
        "Advanced Info Service",
        "Advanced Wireless Network Company Ltd",
        "True Move H Universal Communication Company Ltd",
        "TOT Public Company Limited",
        "Total Access Communications Public Company Ltd",
        "ACeS",
        "Digital Phone Company Ltd",
        "True Corporation"
    ],
    "Togo": [
        "Togo Telecom"
    ],
    "Tokelau": [
        "Teletok"
    ],
    "Tonga": [
        "Tonga Communications Corporation",
        "Shoreline Communication",
        "Digicel (Tonga) Limited"
    ],
    "Trinidad and Tobago": [
        "TSTT",
        "Digicel (Trinidad & Tobago) Limited",
        "LaqTel Ltd"
    ],
    "Tunisia": [
        "Tunisie Telecom",
        "ooredoo Tunisiana"
    ],
    "Turkey": [
        "Turkcell Iletisim Hizmetleri AS",
        "Vodafone Turkey",
        "Aycell"
    ],
    "Turkmenistan": [
        "MTS Turkmenistan",
        "Altyn Asyr"
    ],
    "Turks and Caicos Islands": [
        "Cable & Wireless West Indies Ltd (Turks & Caicos)"
    ],
    "Tuvalu": [
        "Tuvalu Telecom"
    ],
    "Uganda": [
        "Bharti Airtel",
        "Tangerine Uganda Limited",
        "Afrimax Uganda",
        "MTN Uganda",
        "Uganda Telecom Ltd",
        "Unknown",
        "Operational",
        "Not operational"
    ],
    "Ukraine": [
        "Intertelecom LLC",
        "lifecell LLC",
        "Trimob LLC"
    ],
    "United Arab Emirates": [
        "Emirates Telecom Corp"
    ],
    "United Kingdom": [
        "BT Group",
        "Mundio Mobile Limited",
        "Jersey Airtel Ltd",
        "FMS Solutions Ltd",
        "COLT Mobile Telecommunications Limited",
        "Internet Computer Bureau Limited",
        "Vodafone",
        "BT OnePhone (UK) Ltd",
        "Tismi BV",
        "Hay Systems Ltd",
        "TalkTalk Communications Limited",
        "FleXtel Limited",
        "Cloud9",
        "Teleware plc",
        "Hutchison 3G UK Ltd",
        "Telesign Mobile Limited",
        "Icron Network Limited",
        "Stour Marine Limited",
        "Truphone",
        "Lycamobile UK Limited",
        "Teleena UK Limited",
        "Marathon Telecom Limited",
        "(aq) Limited",
        "EE",
        "Sure Isle of Man Ltd",
        "Synectiv Ltd",
        "Virgin Media",
        "Gamma Telecom Holdings Ltd",
        "JT Group Limited",
        "UK Broadband Limited",
        "Shyam Telecom UK Ltd",
        "Limitless Mobile Ltd",
        "Sure (Guernsey) Limited",
        "CESG",
        "Sky UK Limited",
        "Manx Telecom",
        "AMSUK Ltd",
        "Home Office",
        "Airwave Solutions Ltd",
        "Vodafone United Kingdom"
    ],
    "United States": [
        "Verizon Wireless",
        "AT&T Mobility",
        "Union Telephone Company",
        "ETEX Communications, LP",
        "Matanuska Telephone Association, Inc",
        "Alaska Communications",
        "Consolidated Telcom",
        "New Mexico RSA 4 East LP",
        "PTI Pacifica Inc",
        "Sprint Corporation",
        "Carolina West Wireless",
        "Teleguam Holdings, LLC",
        "T-Mobile US",
        "West Central Wireless",
        "T-Mobile",
        "T-Mobile USA",
        "NEP Cellcorp Inc",
        "iSmart Mobile, LLC",
        "Smith Bagley, Inc",
        "Cellular Network Partnership",
        "TX-11 Acquisition, LLC",
        "Wave Runner LLC",
        "GCI Communications Corp",
        "Numerex",
        "Viaero Wireless",
        "NewCore Wireless LLC",
        "Public Service Cellular Inc",
        "Nsight",
        "Transaction Network Services",
        "Iowa Wireless Services LLC",
        "Hilliary Communications",
        "TX-10, LLC and Central Louisiana Cellular, LLC (MTPCS)",
        "Inland Cellular Telephone Company",
        "NewCell Inc",
        "Elkhart Telephone Co",
        "Nsighttel Wireless LLC",
        "Agri-Valley Communications",
        "Jasper Technologies",
        "Limitless Mobile, LLC",
        "Cross Valiant Cellular Partnership",
        "Arctic Slope Telephone Association Cooperative",
        "US Cellular",
        "East Kentucky Network, LLC",
        "Lynch 3G Communications Corporation",
        "Iowa Wireless Services",
        "D D Inc",
        "PinPoint Communications Inc",
        "Telecom North America Mobile, Inc",
        "Aeris Communications, Inc",
        "TX RSA 15B2, LP",
        "Advantage Cellular Systems, Inc",
        "Cable & Communications Corporation",
        "James Valley Wireless, LLC",
        "Copper Valley Wireless",
        "UBET Wireless",
        "Globalstar",
        "Evolve Cellular Inc",
        "Mid-Tex Cellular Ltd",
        "Chariton Valley Communications",
        "Missouri RSA 5 Partnership",
        "Americell PA 3 Partnership",
        "Commnet Wireless",
        "Thumb Cellular LP",
        "Space Data Corporation",
        "Pine Telephone Company",
        "Nex-Tech Wireless",
        "Cross Telephone Company",
        "Wilkes Cellular",
        "Tampnet",
        "ARINC",
        "Cellular South Inc",
        "Cordova Wireless",
        "Nexus Communications, Inc",
        "Leaco Rural Telephone Company Inc",
        "Bug Tussel Wireless LLC",
        "Illinois Valley Cellular",
        "Sagebrush Cellular, Inc",
        "Stelera Wireless",
        "General Communication Inc",
        "New Dimension Wireless Ltd",
        "Iowa RSA No 2 LP",
        "Northwest Missouri Cellular LP",
        "RSA 1 LP",
        "Bluegrass Cellular LLC",
        "Panhandle Telecommunication Systems Inc",
        "Vitelcom Cellular Inc",
        "Mosaic Telecom",
        "Ligado Networks",
        "Commnet Wireless LLC",
        "OTZ Communications, Inc",
        "North Dakota Network Co",
        "TerreStar Networks, Inc",
        "Standing Rock Telecommunications",
        "United Wireless",
        "Metro by T-Mobile",
        "Pine Belt Cellular Inc",
        "GreenFly LLC",
        "TeleBEEPER of New Mexico",
        "Midwest Network Solutions Hub LLC",
        "Northeast Wireless Networks LLC",
        "MainePCS LLC",
        "Proximiti Mobility Inc",
        "Telalaska Cellular",
        "Flat Wireless LLC",
        "Altiostar Networks, Inc",
        "Coleman County Telephone Cooperative, Inc",
        "Sonus Networks",
        "Uintah Basin Electronic Telecommunications",
        "Globecomm Network Services Corporation",
        "GigSky",
        "SI Wireless LLC",
        "Enhanced Telecommmunications Corp",
        "Lycamobile USA Inc",
        "Big River Broadband, LLC",
        "LigTel Communications",
        "VTel Wireless",
        "Chariton Valley Communications Corporation, Inc",
        "Infrastructure Networks, LLC",
        "Cross Wireless",
        "Fuego Wireless",
        "CoverageCo",
        "Adams Networks Inc",
        "South Georgia Regional Information Technology Authority",
        "ClearSky Technologies, Inc",
        "Cleveland Unlimited, Inc",
        "RSA1 Limited Partnership",
        "Limitless Mobile LLC",
        "Aspenta International, Inc",
        "Central LTE Holdings",
        "Clear Stream Communications, LLC",
        "RTC Communications LLC",
        "Nemont Communications, Inc",
        "Triangle Communication System Inc",
        "Wes-Tex Telecommunications, Ltd",
        "FTC Communications LLC",
        "Mid-Rivers Telephone Cooperative",
        "Eltopia Communications, LLC",
        "Silver Star Communications",
        "TrustComm, Inc",
        "WUE Inc",
        "Great Plains Communications, Inc",
        "Buffalo-Lake Erie Wireless Systems Co, LLC",
        "Northern Michigan University",
        "GlobeTouch Inc",
        "NetGenuity, Inc",
        "365 Wireless LLC",
        "TGS, LLC",
        "Wireless Partners, LLC",
        "Great North Woods Wireless LLC",
        "Southern Communications Services",
        "Artemis Networks LLC",
        "Redzone Wireless",
        "Gila Electronics",
        "Cirrus Core Networks",
        "Bristol Bay Telephone Cooperative",
        "Santel Communications Cooperative, Inc",
        "Kings County Office of Education",
        "Onvoy Spectrum, LLC",
        "Flat Wireless, LLC",
        "GigSky Mobile, LLC",
        "Albemarle County Public Schools",
        "Circle Gx",
        "Flat West Wireless, LLC",
        "Hewlett-Packard Communication Services, LLC",
        "Webformix",
        "M&A Technology, Inc",
        "IOSAZ Intellectual Property LLC",
        "Mark Twain Communications Company",
        "Premier Holdings LLC",
        "Tennessee Wireless",
        "Cross Wireless LLC",
        "Cambridge Telephone Company Inc",
        "Nucla-Naturita Telephone Company",
        "Manti Tele Communications Company, Inc",
        "Country Wireless",
        "Speedwavz LLP",
        "Vivint Wireless, Inc",
        "Mercury Network Corporation",
        "Nextel Communications"
    ],
    "Uruguay": [
        "AM Wireless Uruguay SA"
    ],
    "Uzbekistan": [
        "Buztel",
        "Uzmacom",
        "Unitel LLC",
        "Coscom",
        "Universal Mobile Systems"
    ],
    "Vanuatu": [
        "Telecom Vanuatu Ltd",
        "Digicel Vanuatu Ltd",
        "WanTok Vanuatu Ltd"
    ],
    "Viet Nam": [
        "Vietnam Mobile Telecom Services Company",
        "Vietnam Telecom Services Company",
        "S-Telecom",
        "Viettel Telecom",
        "EVN Telecom",
        "GTEL Mobile JSC",
        "Indochina Telecom"
    ],
    "Yemen": [
        "Spacetel Yemen",
        "Y"
    ],
    "Zambia": [
        "Bharti Airtel",
        "MTN Group",
        "Zambia Telecommunications Company Ltd"
    ],
    "Zimbabwe": [
        "Net*One Cellular (Pvt) Ltd",
        "Telecel Zimbabwe (PVT) Ltd",
        "Econet Wireless"
    ],
    "International operators": [
        "Telenor Maritime AS",
        "BebbiCell AG",
        "AeroMobile AS",
        "OnAir Switzerland Sarl",
        "Cisco Systems, Inc",
        "JT Group Limited",
        "AT&T Mobility",
        "Intermatica",
        "Wins Limited",
        "MediaLincc Ltd",
        "Voxbone",
        "Telecom Italia Mobile",
        "Monaco Telecom",
        "GDSP (Vodafone's Global Data Service Platform)",
        "Orange SA",
        "MegaFon",
        "Smart Communications",
        "Globecomm Network Services",
        "Azerfon",
        "Transatel",
        "MTX Connect Ltd",
        "BodyTrace Netherlands BV",
        "DCN Hub ehf",
        "EMnify GmbH",
        "AT&T Inc",
        "Advanced Wireless Network Company Limited",
        "Telecom26 AG",
        "Ooredoo",
        "Communication for Devices in Sweden AB",
        "Zain Kuwait",
        "EchoStar Mobile",
        "VisionNG",
        "Teleena Holding BV",
        "Beezz Communication Solutions Ltd",
        "European Telecommunications Standards Institute",
        "SAP",
        "MessageBird BV",
        "OneWeb",
        "MTN Management Services",
        "GloTel BV",
        "Syniverse Technologies, LLC",
        "UN Office for the Coordination of Humanitarian Affairs (OCHA)"
    ]
}